...
#define SET_FREEBIT(chunk) ( *(unsigned *)&(chunk)->s.r |=  0x1 ) // sets to 1 (if anything is ORed with 1 it is 1)
#define CLR_FREEBIT(chunk) ( *(unsigned *)&(chunk)->s.r &= ~0x1 ) // sets to 0 (ANDed with 0)
 ...
 */
#define TOCHUNK(vp) (-1 + (CHUNK *)(vp)) // gives you a chunk from pointer
#define FROMCHUNK(chunk) ((void *)(1 + (chunk))) // gives you pointer from chunk
